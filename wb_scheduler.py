        ...
        for acc in list(self.pool.accounts):
            ...
            if acc.realm == "intl":
                if acc.can_daily_chat():
                    self.log(f"Phát hiện tài khoản Quốc Tế [{uid8}] hôm nay chưa hoạt động, thực hiện điểm danh hoạt động hàng ngày...")
                    res = acc.daily_chat()
                    if res.get("ok"):
                        daily_chat_count += 1
                        self.log(f"✓ Tài khoản [{uid8}] điểm danh hoạt động hàng ngày thành công")
                    else:
                        self.log(f"! Tài khoản [{uid8}] điểm danh hoạt động hàng ngày thất bại: {res.get('error') or res.get('msg')}")
                    time.sleep(1.5)

        self.log(f"Hoàn tất kiểm tra: Token bảo lưu {refreshed_count}, điểm danh hàng ngày {checkin_count}, mèo hàng ngày {travel_count}, hoạt động Quốc Tế {daily_chat_count}")
